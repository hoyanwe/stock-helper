# -*- coding: utf-8 -*-
"""Copy of US-stock-helper

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1UPqu2PM7KX9HiTvltwp4pyHq9f_jFrOv
"""

# =========================
# Batch Equity Forecasts (Robust Colab Version - Final, improved Kalman compatibility)
# =========================
import warnings
...
# ========= Econ cycle (categorical, fixed dtype) =========
...
def prophet_forecast(close: pd.Series, horizon_days: int = 180) -> pd.DataFrame:
    ...
    if len(close) < 10:
        ...
        future_dates = pd.date_range(close.index.max() + timedelta(days=1), periods=horizon_days, freq="D")
...
# ========= Kalman local linear trend (robust across statsmodels versions) =========
def kalman_trend_and_forecast(close: pd.Series, horizon_days: int = 180):
    ...
    st = None
    # 1) prefer res.smoothed_state (numpy array-like) if present
    if hasattr(res, "smoothed_state"):
        try:
            st = res.smoothed_state  # typically shape (k_states, nobs)
        except Exception:
            st = None
    # 2) fallback to res.states.smoothed (DataFrame) if available
    if st is None and hasattr(res, "states"):
        try:
            ...
            st = None
    # 3) fallback to older attribute name if exists
    if st is None and hasattr(res, "level_smoothed"):
        try:
            st = res.level_smoothed
        except Exception:
            st = None

    # Extract level series from whatever 'st' is
    level = None
    try:
        if st is None:
    ...
    # 历史拟合（fittedvalues）
    fitted = res.fittedvalues.reindex(close.index)
    ...
    try:
        ...
        out = pd.DataFrame({
            "ds": pd.date_range(close.index.max() + timedelta(days=1), periods=horizon_days, freq="D"),
    ...
    return level, fitted, out

# ========= ARIMA on returns (robust) =========
...
# ========= Lasso factor forecast (rolling) =========
def lasso_factor_forecast_rolling(features: pd.DataFrame, horizon_days: int = 180) -> pd.DataFrame:
    df = features.copy()
    num_cols = df.select_dtypes(include=[np.number]).columns
    df[num_cols] = df[num_cols].replace([np.inf, -np.inf], np.nan).ffill().bfill()
    df["logret"] = np.log(df["Close"]).diff()
    X = df.drop(columns=["logret"], errors="ignore").select_dtypes(include=[np.number])
    y = df["logret"]
    valid = y.dropna().index.intersection(X.dropna().index)
    X = X.loc[valid].dropna()
    y = y.loc[X.index]
    last_price = float(df["Close"].iloc[-1])
    future_dates = pd.date_range(df.index.max() + timedelta(days=1), periods=horizon_days, freq="D")
    if len(y) < 60 or X.shape[1] < 1:
        return pd.DataFrame({"ds": future_dates,
                             "yhat": last_price,
                             "yhat_lower": last_price,
                             "yhat_upper": last_price})
    ...
    last_num_row = df.iloc[-1][num_cols]
    preds, lower, upper = [], [], []
    price = last_price
    for step in range(horizon_days):
        tech = compute_technicals_from_close(close_sim)
        row = last_num_row.copy()
        for col in ["Return","Volatility20","MA20","MA50","MA200","ROC20","RSI14","MACD","MACDsig","MACDhist","Close","Open","High","Low","Adj Close","Volume"]:
        ...
        try:
            pred_logret = float(model.predict(row.values.reshape(1, -1))[0])
        except Exception:
            pred_logret = 0.0
        ...
        preds.append(price)
        if sigma > 0:
            cum_lower = float(np.exp((step+1)*(-1.96*sigma)))
            cum_upper = float(np.exp((step+1)*(+1.96*sigma)))
            lower.append(last_price * cum_lower)
            upper.append(last_price * cum_upper)
        else:
...
def compute_feature_importance(features_df: pd.DataFrame) -> pd.DataFrame:
    X = features_df.drop(columns=["Close"], errors="ignore").select_dtypes(include=[np.number])
    y = features_df["Close"]
    X = X.dropna(axis=1, how="all").replace([np.inf, -np.inf], np.nan).ffill().bfill().dropna()
    if X.shape[0] == 0:
        raise ValueError("清洗后样本数为 0，跳过 Lasso")
    y = y.loc[X.index]
    model = LassoCV(cv=5, random_state=0).fit(X, y)
    importance = pd.DataFrame({"feature": X.columns, "importance": np.abs(model.coef_)}).sort_values("importance", ascending=False)
    return importance
...
def ensemble_predictions(close: pd.Series,
    ...
    close_tail = close.tail(max_window)
    prop_hist = prophet_fc.set_index("ds").reindex(close.index)["yhat"].tail(max_window)
    e_prop = rmse(close_tail, prop_hist)
    kal_hist = kalman_fitted.reindex(close.index).tail(max_window)
    e_kal = rmse(close_tail, kal_hist)
    arima_hist = close.shift(1).tail(max_window)
    e_ari = rmse(close_tail, arima_hist)
    lasso_hist = close.shift(1).tail(max_window)
    e_las = rmse(close_tail, lasso_hist)
    ...
    H = min(len(d) for d in dfs)
    horizon_dates = pd.date_range(close.index.max() + timedelta(days=1), periods=H, freq="D")
...
for tk in tqdm(TICKERS, desc="Processing"):
    try:
        ...
        kal_trend, kal_fitted, fc_kalman = kalman_trend_and_forecast(close, horizon_days=180)
        kal_trend = kal_trend.to_frame(name="KalmanTrend")
        kal_trend["Ticker"] = tk
        all_kalman_trend.append(kal_trend.reset_index().rename(columns={"index":"Date"}))
        fc_arima = arima_returns_forecast(close, horizon_days=180)
        try:
            fc_lasso = lasso_factor_forecast_rolling(merged, horizon_days=180)
        except Exception as e:
            last_price = float(close.iloc[-1])
            future_dates = pd.date_range(close.index.max() + timedelta(days=1), periods=180, freq="D")
            fc_lasso = pd.DataFrame({"ds": future_dates,
                                     "yhat": last_price,
                                     "yhat_lower": last_price,
                                     "yhat_upper": last_price})
        window = min(60, max(10, len(close)//2))
        fc_ens = ensemble_predictions(close, fc_prophet, kal_fitted, fc_kalman, fc_arima, fc_lasso, backtest_window=window)
        fc_ens["Ticker"] = tk
